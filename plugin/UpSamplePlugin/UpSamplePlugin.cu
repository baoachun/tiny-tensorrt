...
Dims UpSamplePlugin::getOutputDimensions(int index, const Dims *inputs,
                                         int nbInputDims) {
  // spdlog::info("getOutputDimensions...");
  // spdlog::info("input dimemsion:
  // {},{},{}",inputs[0].d[0],inputs[0].d[1],inputs[0].d[2]);
  // spdlog::info("index:{}, nbInputDims:{},
  // mScale:{}",index,nbInputDims,mScale);
  ...
  return Dims3(inputs[0].d[0], (int)(inputs[0].d[1] * mScale),
               (int)(inputs[0].d[2] * mScale));
}

void UpSamplePlugin::configureWithFormat(const Dims *inputDims, int nbInputs,
...
IPluginV2 *UpSamplePlugin::clone() const { return new UpSamplePlugin(mScale); }
...
IPluginV2 *
UpSamplePluginCreator::createPlugin(const char *layerName,
                                    const PluginFieldCollection *fc) {
  return nullptr;
}

// deserialization plugin implementation
IPluginV2 *UpSamplePluginCreator::deserializePlugin(const char *layerName,
...
template <typename Dtype>
void UpSamplePlugin::forwardGpu(const Dtype *input, Dtype *output, int N, int C,
                                int H, int W) {

  int numElem = N * C * H * W;
  upscale<<<(numElem + NUM_THREADS - 1) / NUM_THREADS, NUM_THREADS>>>(
      input, output, numElem, mScale, C, H, W);
}

int UpSamplePlugin::enqueue(int batchSize, const void *const *inputs,
  ...
  case DataType::kFLOAT:
    forwardGpu<float>((const float *)inputs[0], (float *)outputs[0], batchSize,
