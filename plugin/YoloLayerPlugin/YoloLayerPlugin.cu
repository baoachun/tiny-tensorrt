...
// YOLO 608
YoloKernel yolo1_608 = {19, 19, {116, 90, 156, 198, 373, 326}};
YoloKernel yolo2_608 = {38, 38, {30, 61, 62, 45, 59, 119}};
YoloKernel yolo3_608 = {76, 76, {10, 13, 16, 30, 33, 23}};

// YOLO 416
YoloKernel yolo1_416 = {13, 13, {116, 90, 156, 198, 373, 326}};
YoloKernel yolo2_416 = {26, 26, {30, 61, 62, 45, 59, 119}};
YoloKernel yolo3_416 = {52, 52, {10, 13, 16, 30, 33, 23}};
...
Dims YoloLayerPlugin::getOutputDimensions(int index, const Dims *inputs,
  ...
  for (const auto &yolo : mYoloKernel)
    totalCount += yolo.width * yolo.height * CHECK_COUNT * sizeof(Detection) /
                  sizeof(float);
...
int YoloLayerPlugin::initialize() {
  ...
  for (const auto &yolo : mYoloKernel)
    totalCount +=
        (LOCATIONS + 1 + mClassCount) * yolo.width * yolo.height * CHECK_COUNT;
  CUDA_CHECK(cudaHostAlloc(&mInputBuffer, totalCount * sizeof(float),
  ...
  CUDA_CHECK(cudaHostAlloc(&mOutputBuffer,
                           sizeof(float) + totalCount * sizeof(Detection),
                           cudaHostAllocDefault));
  return 0;
}
...
int YoloLayerPlugin::enqueue(int batchSize, const void *const *inputs,
  ...
  // CPU
  // forwardCpu((const float *const *)inputs,(float
  // *)outputs[0],stream,batchSize);
  return 0;
}

size_t YoloLayerPlugin::getSerializationSize() const {
  return sizeof(mClassCount) + sizeof(mYolo3NetSize) + sizeof(mKernelCount) +
         sizeof(YoloKernel) * mYoloKernel.size();
}
...
void YoloLayerPlugin::forwardCpu(const float *const *inputs, float *outputs,
  ...
  auto Logist = [=](float data) { return 1. / (1. + exp(-data)); };

  int totalOutputCount = 0;
  int i = 0;
  int totalCount = 0;
  for (const auto &yolo : mYoloKernel) {
    totalOutputCount += yolo.width * yolo.height * CHECK_COUNT *
  ...
  for (int idx = 0; idx < batchSize; idx++) {
    i = 0;
    float *inputData =
        (float *)mInputBuffer; // + idx *totalCount; //if create more batch size
    for (const auto &yolo : mYoloKernel) {
      int size = (LOCATIONS + 1 + mClassCount) * yolo.width * yolo.height *
                 CHECK_COUNT;
      CUDA_CHECK(cudaMemcpyAsync(inputData, (float *)inputs[i] + idx * size,
      ...
      for (int j = 0; j < stride; ++j) {
        for (int k = 0; k < CHECK_COUNT; ++k) {
          ...
          for (int c = 0; c < mClassCount; ++c) {
            float cProb =
                Logist(inputData[beginIdx + (5 + c) * stride]) * objProb;
            if (cProb > maxProb) {
              maxProb = cProb;
              classId = c;
            }
          ...
          if (classId >= 0) {
            ...
            det.bbox[1] =
                (row + Logist(inputData[beginIdx + stride])) / yolo.height;
            det.bbox[2] =
                exp(inputData[beginIdx + 2 * stride]) * yolo.anchors[2 * k];
            det.bbox[3] =
                exp(inputData[beginIdx + 3 * stride]) * yolo.anchors[2 * k + 1];
            det.classId = classId;
            det.prob = maxProb;
    ...
    auto data = (float *)mOutputBuffer; // + idx*(totalOutputCount + 1); //if
                                        // create more batch size
    float *begin = data;
    // copy count;
    ...
    CUDA_CHECK(cudaMemcpyAsync(
        outputs, begin, sizeof(float) + result.size() * sizeof(Detection),
...
__global__ void CalDetection(const float *input, float *output, int noElements,
                             ...
                             const float anchors[CHECK_COUNT * 2], int classes,
                             int outputElem) {
  ...
  const float *curInput =
      input + bnIdx * ((LOCATIONS + 1 + classes) * stride * CHECK_COUNT);
  ...
  for (int k = 0; k < CHECK_COUNT; ++k) {
    ...
    if (classId >= 0) {
      ...
      char *data =
          (char *)curOutput + sizeof(float) + resCount * sizeof(Detection);
...
void YoloLayerPlugin::forwardGpu(const float *const *inputs, float *output,
  ...
  for (unsigned int i = 0; i < mYoloKernel.size(); ++i) {
    ...
    outputElem += yolo.width * yolo.height * CHECK_COUNT * sizeof(Detection) /
                  sizeof(float);
  }
  ...
  for (unsigned int i = 0; i < mYoloKernel.size(); ++i) {
    ...
    CalDetection<<<(yolo.width * yolo.height * batchSize + 512 - 1) / 512,
                   512>>>(inputs[i], output, numElem, yolo.width, yolo.height,
                          (float *)devAnchor, mClassCount, outputElem);
  }
...
IPluginV2 *
YoloLayerPluginCreator::createPlugin(const char *layerName,
                                     const PluginFieldCollection *fc) {
  return nullptr;
}

// deserialization plugin implementation
IPluginV2 *YoloLayerPluginCreator::deserializePlugin(const char *layerName,
