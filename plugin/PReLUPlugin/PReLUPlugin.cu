...
const char *G_PRELU_NAME =
    "PReLU_TRT"; // plugin_name = plugin_type + plugin_namespace
...
__global__ void PReLUForward(const int n, const int channels, const int dim,
                             const Ftype *slope_data, const Ftype *in,
                             Ftype *out, const Ftype zero,
...
cudaError_t Forward_gpu(const int count, const int channels, const int dim,
                        const Ftype *mDeviceKernel, const Ftype *bottom_data,
                        Ftype *top_data, const Ftype zero, const int div_factor,
                        const cudaStream_t stream) {
  PReLUForward<<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, stream>>>(
      count, channels, dim, mDeviceKernel, bottom_data, top_data, zero,
      div_factor);
...
PReLUPlugin::PReLUPlugin(const nvinfer1::Weights *weights, int nbWeights) {
  ...
  memcpy(const_cast<void *>(mWeights.values), weights[0].values,
...
PReLUPlugin::PReLUPlugin(const void *data, size_t length) {
  ...
  mWeights.values = malloc(mWeights.count * type2size(mWeights.type));
  memcpy(const_cast<void *>(mWeights.values), d,
         mWeights.count * type2size(mWeights.type));
...
size_t PReLUPlugin::getSerializationSize() const {
  return sizeof(mNbInputChannels) + sizeof(mNbInputWidth) +
         sizeof(mNbInputHeight) + sizeof(mDataType) + sizeof(mWeights.count) +
...
size_t PReLUPlugin::getWorkspaceSize(int maxBatchSize) const { return 0; }

int PReLUPlugin::enqueue(int batchSize, const void *const *inputs,
                         void **outputs, void *workspace, cudaStream_t stream) {
  const int count =
      batchSize * mNbInputChannels * mNbInputWidth * mNbInputHeight;
  ...
  if (mDataType == nvinfer1::DataType::kFLOAT) {
    ...
    CUDA_CHECK(Forward_gpu(
        count, channels, dim, reinterpret_cast<const float *>(mDeviceKernel),
        reinterpret_cast<const float *>(inputs[0]),
        reinterpret_cast<float *>(outputs[0]), zerof, div_factor, stream));
  }
#ifdef FP16_PRELU
  else {
    const __half zeroh = __half(0.0f);
    CUDA_CHECK(Forward_gpu(
        count, channels, dim, reinterpret_cast<const __half *>(mDeviceKernel),
        reinterpret_cast<const __half *>(inputs[0]),
        reinterpret_cast<__half *>(outputs[0]), zeroh, div_factor, stream));
  }
#else
  else {
...
const char *PReLUPlugin::getPluginType() const { return G_PRELU_TYPE; }

const char *PReLUPlugin::getPluginVersion() const { return G_PLUGIN_VERSION; }
...
PReLUPluginCreator::PReLUPluginCreator() {
  mPluginAttributes.emplace_back(nvinfer1::PluginField(
      "weights", nullptr, nvinfer1::PluginFieldType::kFLOAT32, 1));
  mPluginAttributes.emplace_back(nvinfer1::PluginField(
      "nbWeight", nullptr, nvinfer1::PluginFieldType::kINT32, 1));
...
nvinfer1::IPluginV2 *
PReLUPluginCreator::createPlugin(const char *name,
...
nvinfer1::IPluginV2 *PReLUPluginCreator::deserializePlugin(
